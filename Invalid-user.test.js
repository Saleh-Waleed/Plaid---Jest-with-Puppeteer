...
describe('Example', () => {
    ...
    beforeAll(async () => {
        jest.setTimeout(25000); // change timeout to 25 seconds
...
it('Adding the Bank with Valid User Name', async () => {
  ...
  const iframeElement = await page.$("#plaid-link-iframe-1");
  const iframeContent = await iframeElement.contentFrame();
  // Find the input field and type "user_good12345"
  await iframeContent.type('#aut-input-0', 'user_good12345');
  });
  ...
  it('Adding the Bank with Valid Password', async () => {
    ...
    const iframeContent = await iframeElement.contentFrame();
    // Find the input field and type "user_good12345"
    await iframeContent.type('#aut-input-1', 'pass_good12345');
    });
